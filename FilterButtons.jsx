...
const FilterButtons = ({ filterData, flag }) => {
    ...
    return (
        ...
        Cursor-pointer hover:border-gray-400 '>
            <p className='text-base tracking-wide text-text-color flex items-center
           gap-2' onClick={() => setFilterMenu(!filterMenu)}>
                ...
                <IoChevronDown className={`text-base text-text-Color duration-150
            transition-all ease-in-out ${filterMenu ? "rotate-180 " : "rotate-0"}`} />
            ...
            {filterData && filterMenu && (
                <motion.div initial={{ opacity: 0, y: 50 }}
                    ...
                    exit={{ opacity: 0, y: 50 }}
                    className=" w-48 z-50 backdrop-blur-sm max-h-44
            overflow-y-scroll scrollbar-thin scrollbar-track-gray-200
            scrollbar-thumb-gray-400 py-2 flex-col rounded-md shadow-md absolute
            top-8 left-0">
